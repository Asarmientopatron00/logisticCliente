...
const cells = [
  ...
  },
  {
    ...
    value: (value) => TIPOS_DOCUMENTOS.map((typ) => typ.id === value ? typ.nombre : ''),
    align: 'left',
  },
  {
    id: 'numero_documento',
...
const Cliente = (props) => {
  ...
  const {rows, desde, hasta, ultima_pagina, total, getList, onDelete} = useContext(ClienteContext);
  ...
  const onOpenEditCliente = (row) => {
    setSelected(row);
    setAccion(ACTIONS.editar);
    setShowForm(true);
  };

  const onOpenViewCliente = (row) => {
    setSelected(row);

...
import BodegaCreador from './BodegaCreador';
import {
  UPDATE,
  CREATE,
  DELETE,
...
const Bodega = (props) => {
  ...
  const {rows, desde, hasta, ultima_pagina, total, getList, onDelete} = useContext(BodegaContext);
  ...
  const onOpenEditBodega = (row) => {
    setSelected(row);
    setAccion(ACTIONS.editar);
    setShowForm(true);
  };

  const onOpenViewBodega = (row) => {
    setSelected(row);
    setAccion(ACTIONS.ver);
    setShowForm(true);
  };

  const onDeleteBodega = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar La Bodega?',
    ...
    <Box sx={mainStyles.root}>
      <Paper sx={mainStyles.paper}>
        <EnhancedTableToolbar
          onOpenAddBodega={onOpenAddBodega}

...
import TipoProductoTerrestreCreador from './TipoProductoTerrestreCreador';
import {
  UPDATE,
  CREATE,
  DELETE,
...
const EnhancedTableToolbar = (props) => {
  const {
    onOpenAddTipoProductoTerrestre,
    queryFilter,
    nombre,
    limpiarFiltros,
    titulo
      ...
      <>
        <Box sx={mainStyles.titleTop}>
          ...
          <Box sx={mainStyles.horizontalBottoms}>
            <Tooltip
              title='Crear Tipo Producto Terrestre'
              onClick={onOpenAddTipoProductoTerrestre}>
              <IconButton
...
const titulo = 'Tipos Productos Log. Terrestre';

const TipoProductoTerrestre = (props) => {
  ...
  const {rows, desde, hasta, ultima_pagina, total, getList, onDelete} = useContext(TipoProductoTerrestreContext);
  ...
  const onOpenEditTipoProductoTerrestre = (row) => {
    setSelected(row);
    setAccion(ACTIONS.editar);
    setShowForm(true);
  };

  const onOpenViewTipoProductoTerrestre = (row) => {
    setSelected(row);
    setAccion(ACTIONS.ver);
    setShowForm(true);
  };

  const onDeleteTipoProductoTerrestre = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar El Tipo Producto Terrestre?',
  ...
  const onOpenAddTipoProductoTerrestre = () => {
    setSelected(0);
    ...
    <Box sx={mainStyles.root}>
      <Paper sx={mainStyles.paper}>
        <EnhancedTableToolbar
          onOpenAddTipoProductoTerrestre={onOpenAddTipoProductoTerrestre}
                        ...
                        >
                          <TableCell align='center' sx={mainStyles.acciones}>
                            <Tooltip title={'Editar'}>
                              <Edit
                                onClick={() => onOpenEditTipoProductoTerrestre(row)}
                                sx={{...mainStyles.generalIcons, ...mainStyles.editIcon}}
                              />
                            </Tooltip>
                            <Tooltip title={'Ver'}>
                              <Visibility
                                onClick={() => onOpenViewTipoProductoTerrestre(row)}
                                sx={{...mainStyles.generalIcons, ...mainStyles.visivilityIcon}}
                              />
                            </Tooltip>
                            <Tooltip title={'Eliminar'}>
                              <Delete
                                onClick={() => onDeleteTipoProductoTerrestre(row.id)}

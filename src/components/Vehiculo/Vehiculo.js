...
const Vehiculo = (props) => {
  ...
  const {rows, desde, hasta, ultima_pagina, total, getList, onDelete} = useContext(VehiculoContext);
  ...
  const onOpenEditVehiculo = (row) => {
    setSelected(row);
    setAccion(ACTIONS.editar);
    setShowForm(true);
  };

  const onOpenViewVehiculo = (row) => {
    setSelected(row);
    setAccion(ACTIONS.ver);
    setShowForm(true);
  };

  const onDeleteVehiculo = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar El Vehiculo?',
    ...
    <Box sx={mainStyles.root}>
      <Paper sx={mainStyles.paper}>
        <EnhancedTableToolbar
          onOpenAddVehiculo={onOpenAddVehiculo}
